...
namespace danielCherrin_CarRentalApp
{
    public partial class frm_AlterContract : Form
    {
        ...
        public frm_AlterContract(Contract alterContract)
        {
            savedContract = alterContract;
            InitializeComponent();
            ...
            contractObj.contractNo = savedContract.contractNo;
            contractObj.VIN = savedContract.VIN;
            contractObj.empNo = savedContract.empNo;
            contractObj.customerNo = savedContract.customerNo;
            txt_EmpNo.Text = contractObj.empNo.ToString();
            txt_CustomerNo.Text = contractObj.customerNo.ToString();
            txt_VIN.Text = savedContract.VIN;
            dtp_PickupDate.Text = savedContract.pickupDate;
            dtp_PickupTime.Text = savedContract.pickupTime;
            dtp_ReturnDate.Text = savedContract.returnDate;
            dtp_ReturnTime.Text = savedContract.returnTime;
            txt_Discount.Text = savedContract.discount.ToString();
            txt_Cost.Text = savedContract.cost.ToString();
            txt_OdoOutbound.Text = savedContract.odoReadingOutbound.ToString();
            txt_OdoInbound.Text = savedContract.odoReadingInbound.ToString();
            txt_ObOut.Text = savedContract.observationsOut;
            txt_ObIn.Text = savedContract.observationsIn;
        }
            ...
            if (verifyContract())
            {
                string str_QuerySelect =
                    ...
                    "( " +
                        ...
                        "AND contractNo != "+contractObj.contractNo+" "+
                    ") " +
                    "THEN CAST(1 AS BIT) " +
                ...
                if (!Convert.ToBoolean(dta.Rows[0][0].ToString()))
                {
                    ...
                    if (result == System.Windows.Forms.DialogResult.Yes)
                    {
                        ...
                        string myInsert = "UPDATE [dbo].[Contracts] " +
                                                    "SET [empNo] = " + contractObj.empNo +
                                                    ",[customerNo] = " + contractObj.customerNo +
                                                    ",[VIN] = '" + contractObj.VIN + "'" +
                                                    ",[pickupDate] = CAST('" + contractObj.pickupDate + "' AS DATE) " +
                                                    ",[pickupTime] = CAST('" + contractObj.pickupTime + "' AS TIME) " +
                                                    ",[returnDate] = CAST('" + contractObj.returnDate + "' AS DATE) " +
                                                    ",[returnTime] = CAST('" + contractObj.returnTime + "' AS TIME) " +
                                                    ",[discount] = " + contractObj.discount +
                                                    ",[cost] = " + contractObj.cost +
                                                    ",[odoReadingOutbound] = " + contractObj.odoReadingOutbound +
                                                    ",[odoReadingInbound] = " + contractObj.odoReadingInbound +
                                                    ",[observationsOut] = '" + contractObj.observationsOut +"' "+
                                                    ",[observationsIn] = '" + contractObj.observationsIn +"' " +
                                          " WHERE " +
                                                    "[contractNo] = "+contractObj.contractNo+"; ";
                        ...
                        MessageBox.Show("Contract succesfully altered in database");
                        this.Dispose();
                    }
                }
                else
